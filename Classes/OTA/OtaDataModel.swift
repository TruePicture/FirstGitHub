...
public class OtaDataModel {
    public var type: OtaDataType
    public var data: Data
    
    public var otaAddressData: Data!
    public var otaData: Data!
    public var crcData: Data!
    public var sections = [OtaDataSection]()
    
    // 这两个也是为了兼容Nordic平台ota
    public var datData: Data!
    public var typeData: Data!
    
    // TLSR ota数据
    public var tlsrOtaDataIndex = 0
    public var tlsrOtaDataPackages: [Data]!
...
//    }
    ...
    public init(type: OtaDataType, data: Data) {
        self.type = type
        self.data = data
    }
...
//            crc[1] = UInt8((crc_t >> 8) & 0xff)
    ...
    func getNordicTypeData() -> Data {
        switch type {
        case .platform:
            return Data([0x01, 0x04])
        case .touchPanel:
            return Data([0x01, 0x10])
        case .heartRate:
            return Data([0x01, 0x20])
        case .picture:
            return Data([0x01, 0x40])
        case .agps:
            return Data([0x00, 0x00])
        case .freeScale:
            return Data([0x01, 0x08])
        case .gps:
            return Data([0x00, 0x00])
        }
    }
    ...
    func getNordicCrcData(for data: Data) -> Data? {
        ...
        var tmpData = Data([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])
        tmpData.append(crc)
        return tmpData
    }
