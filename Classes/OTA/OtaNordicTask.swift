...
class OtaNordicTask: OtaTask {
    ...
    private func enterUpgradeMode() {
        var kl17Data = Data([0, 0, 0, 0, 0])
        var hrData = Data([0, 0, 0, 0, 0])
        var tpData = Data([0, 0, 0, 0, 0])
...
//            weakSelf?.device.delegate = self
    ...
    private func startOta() {
        ...
        var picAdd = Data([0x00, 0x00, 0x00, 0x00])
        for dm in otaDatas {
            if dm.type == .picture {
    ...
    private func sendOtaBinData() {
        ...
        var data = Data([0x08, UInt8(kOtaNordicTaskResponseNum), 0])
        writeDatData(data)
